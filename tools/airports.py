...
# and generates a zones that are given radius away from the runway edges.
# Zones are then printed out as json.
#
# This script is used to generate no-drone-zones around runways.
# In Finland it is forbidden to fly drones closer than 1 km to 
# runway edges, and with limitations from 1 to 3 km range.
# This tool can be used to approximately genereate the
...
CIRCLE_RADIUS = [1000, 3000]   # 1 km and 3 km clearances from runways
RESTR_TYPE = ["R", "R2"]	# Restriction types for JSON
...
with open('lentokentat.json') as json_file:
  ...
  for airport in data['airports']:
    ...
    # Generate circles for each radius / restriction type
    for idx, radius in enumerate(CIRCLE_RADIUS):
      restr = RESTR_TYPE[idx]
      id += "-" + restr
      ...
      airportdata = {"type":"Feature","id":id,"properties":{"name":name,"type":restr},"geometry":geometry,"publ":date,"updated":updated}
